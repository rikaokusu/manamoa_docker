...
def custom_send_mail(request, subject, message, recipient_list, fail_silently=False, ):
    CUSTOM_EMAIL_HOST = request.user.company.email_host
    CUSTOM_EMAIL_PORT = request.user.company.port
    CUSTOM_EMAIL_HOST_USER = request.user.company.host_user # Gmailのアカウント*
    CUSTOM_EMAIL_HOST_PASSWORD = request.user.company.host_password # Gmailの2段階認証用のパスワード*
    CUSTOM_EMAIL_USE_TLS = True
    CUSTOM_EMAIL_FROM_ADDRESS = request.user.company.from_address
    ...
    if request.user.company.smtp_connection_type == "1": # なし
        server = smtplib.SMTP(CUSTOM_EMAIL_HOST, CUSTOM_EMAIL_PORT)

    elif request.user.company.smtp_connection_type == "2": # STARTTLS
        server = smtplib.SMTP(CUSTOM_EMAIL_HOST, CUSTOM_EMAIL_PORT)
        server.starttls()
        server.login(CUSTOM_EMAIL_HOST_USER, CUSTOM_EMAIL_HOST_PASSWORD)
        print("うごいている4？")

    elif request.user.company.smtp_connection_type == "3": # SSL/TLS
