...
@register.filter
def get_training_status_working(queryset, value):
    ...
    # トレーニングの各ステータス数を取得
    # training_manages_taiou = training.training_manage.filter(status=2).count()
    training_manages_taiou = training.training_manage.filter(status="2", user__in=user_id_list).count()

    # print("------------- 対応", training_manages_taiou)

    return training_manages_taiou
