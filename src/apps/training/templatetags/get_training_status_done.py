...
@register.filter
def get_training_status_done(queryset, value):
    ...
    # training_manages_done = training.training_manage.filter(status=3).count()
    training_manages_done = training.training_manage.filter(status="3", user__in=user_id_list).count()

    # print("------------- 完了", training_manages_done)

    return training_manages_done
